 ...
 * Generated class from Pigeon that represents data sent in messages.
 */
data class LoopPlaybackEndEvent (
  /** Indicates whether this is the end of a loop iteration. */
  val isLoopPlaybackEnd: Boolean
) : PlatformVideoEvent()
 {
  companion object {
    fun fromList(pigeonVar_list: List<Any?>): LoopPlaybackEndEvent {
      val isLoopPlaybackEnd = pigeonVar_list[0] as Boolean
      return LoopPlaybackEndEvent(isLoopPlaybackEnd)
    }
  }
  fun toList(): List<Any?> {
    return listOf(
  ...
  override fun equals(other: Any?): Boolean {
    ...
    val other = other as LoopPlaybackEndEvent
    return MessagesPigeonUtils.deepEquals(this.isLoopPlaybackEnd, other.isLoopPlaybackEnd)
  }

  override fun hashCode(): Int {
    var result = javaClass.hashCode()
    result = 31 * result + MessagesPigeonUtils.deepHash(this.isLoopPlaybackEnd)
    return result
  }
}
...
private open class MessagesPigeonCodec : StandardMessageCodec() {
  override fun readValueOfType(type: Byte, buffer: ByteBuffer): Any? {
    return when (type) {
      ...
      135.toByte() -> {
        return (readValue(buffer) as? List<Any?>)?.let {
          LoopPlaybackEndEvent.fromList(it)
        }
      }
      136.toByte() -> {
        return (readValue(buffer) as? List<Any?>)?.let {
